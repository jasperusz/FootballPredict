...
club_value = player_valuations.groupby('current_club_id')['market_value_in_eur'].sum() # Club market value
...
recent_club_position = recent_club_position.reset_index() # Club recent position on Dataset
...
club_win_rate = club_games[club_games['is_win'] == 1].groupby('club_id').size() / \
    club_games.groupby('club_id').size() # Club win rate
...
opponent_value = club_value.copy() # Oponnent market value
...
recent_opponent_position = recent_opponent_position.reset_index() # Opponent recent position on Dataset
